...
- **聴取（音の印象）**で得た**タグ（刺／無／だ）**が、そのサイクルの**仕込みの向き（= T × タグ）**を規定する。
...
上演開始時に、以下のいずれかをダイスで選び、**素材を処理する順番（Order）**として採用する（**Order は円環**）。
...
- **収束先行**：氷 → 焼酎 → 美酢 → 炭酸 → レモン  
    （Order は後段の **テイストフィックス（TF）**で回転し得るが、**仕込み時点**では決めた順に従う。）
    

### 1-2. 方向表（3種類／ダイスで決定）

上演開始時に **方向表T（T0／T1／T2）** をダイスで 1 種選ぶ。以後、**各サイクルの仕込み**で  
**向き（CW／CCW）＝ T ×（その時点のタグ：刺／無／だ）** により回転方向を決定する。
